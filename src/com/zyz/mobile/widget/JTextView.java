...
import com.zyz.mobile.util.SelectionInfo;

/**
 * User: ray Date: 2013-02-01
 * <p/>
 * WARNING: JTextView is designed to be used inside a JScrollView. The selection functionalities
 * have only been tested under the assumption that the parent of the view is a JScrollView.
 * <p/>
 * The functionalities WILL PROBABLY breaks if the textview is not inside a ScrollView.
 */
public class JTextView extends TextView {
	...
	/**
	 * debug tag use for frequent messages. e.g. message when moving pointer
	 */
	@SuppressWarnings("unused")
	private final static String TAG_FREQUENT = "JTEXT_FREQUENT";
	...
	/**
	 * while mSelectionController is for internal use, mSelectionModifier is to expose functionalities
	 * to outside user of JTextView
	 */
	private SelectionModifier mSelectionModifier;
	...
	private void init() {
		...
		mSelectionModifier = new SelectionModifier();
	...
	protected void onAttachedToWindow() {
		...
		if (getParent() instanceof JScrollView) {
			((JScrollView) getParent()).addOnScrollChangedListener(new OnScrollChangedListener() {
				@Override
...
//	}
	...
	private int getHysteresisOffset(int x, int y, int previousOffset) {
		...
		// * getPrimaryHorizontal(offset) will return 0 becasue the next insertion point is on the next line
	...
	/**
	 * get the offsest given the line and the raw x
	 *
	 * @param line the line
	 * @param x    raw x
	 * @return the offset
	...
	/**
	 * @return the selection modifier
	 */
	public SelectionModifier getSelectionModifier() {
		return mSelectionModifier.getInstance();
	}

	/**
	 * give outsiders controls of the selection controller
	 */
	public class SelectionModifier implements Concealable {
		...
		public SelectionModifier getInstance() {
			if (mSelectionModifier == null) {
				mSelectionModifier = new SelectionModifier();
			}
			return mSelectionModifier;
		}
		...
		public boolean isDisplaying() {
			return mSelectionController.isShowing();
		}

		/**
		 * return the text selected by the cursors
		 *
		 * @return String of the text selected by the cursors
		 */
		public String getSelectedText() {
			if (mSelectionController.isShowing()) {
				return getCursorSelection().getSelectedText().toString();
			}
			return "";
		}

		public int getStart() {
			return getCursorSelection().getStart();
		}

		public int getEnd() {
			return getCursorSelection().getEnd();
		}
	}
	...
	private class SelectionCursorController implements ViewTreeObserver.OnTouchModeChangeListener {
		...
		public void snapToSelection() {
			...
			if (mIsShowing) {
				int a = JTextView.this.getCursorSelection().getStart();
				int b = JTextView.this.getCursorSelection().getEnd();
				...
				final int[] coords = mTempCoords;
				int scroll_y = JTextView.this.getScrollYInternal();
				int scroll_x = JTextView.this.getScrollXInternal();

				JTextView.this.getAdjusteStartXY(start, scroll_x, scroll_y, coords);
				startHandle.pointTo(coords[0], coords[1]);

				JTextView.this.getAdjustedEndXY(end, scroll_x, scroll_y, coords);
		...
		public void show(int start, int end) {
			...
			final int[] coords = mTempCoords;
			int scroll_y = JTextView.this.getScrollY();
			int scroll_x = JTextView.this.getScrollX();

			JTextView.this.getAdjusteStartXY(a, scroll_x, scroll_y, coords);
			mStartHandle.show(coords[0], coords[1]);

			JTextView.this.getAdjustedEndXY(b, scroll_x, scroll_y, coords);
			...
			if (mOnCursorStateChangedListener != null) {
				mOnCursorStateChangedListener.onShowCursors(JTextView.this);
			}
		}
		...
		public void hide() {
			if (mIsShowing) {
				JTextView.this.removeSelection();
				mStartHandle.hide();
				mEndHandle.hide();
				mIsShowing = false;

				if (mOnCursorStateChangedListener != null) {
					mOnCursorStateChangedListener.onHideCursors(JTextView.this);
				}
			}
		}
		 ...
		 * @param oldx         the previos x position the cursor pointed to
		...
		public void updatePosition(CursorHandle cursorHandle, int x, int y, int oldx, int oldy) {
			...
			int old_offset =
					  cursorHandle == mStartHandle ?
								 JTextView.this.getCursorSelection().getStart() :
								 JTextView.this.getCursorSelection().getEnd();

			int offset = JTextView.this.getHysteresisOffset(x, y, old_offset);

			if (offset != old_offset) {

				if (cursorHandle == mStartHandle) {
					JTextView.this.getCursorSelection().setStart(offset);
				}
				else {
					JTextView.this.getCursorSelection().setEnd(offset);
				}
				JTextView.this.getCursorSelection().select();
			}

			cursorHandle.pointTo(x, y);

			if (mOnCursorStateChangedListener != null) {
				mOnCursorStateChangedListener.onPositionChanged(JTextView.this, x, y, oldx, oldy);
			}
		}
		...
		private void select(int start, int end) {
			JTextView.this.setSelection(Math.min(start, end), Math.abs(end - start));
		}
	...
	private class CursorHandle extends View {
		...
		public CursorHandle(SelectionCursorController controller) {
			super(JTextView.this.getContext());
			...
			/* My Note
			At first I tried using mContainer = new PopupWindow(JTextView.this.getContext())
		...
		public boolean /*CursorHandle::*/onTouchEvent(MotionEvent event) {
			...
			switch (event.getAction()) {
				case MotionEvent.ACTION_DOWN: {
					...
					mIsDragging = true;
					if (JTextView.this.mOnCursorStateChangedListener != null) {
						JTextView.this.mOnCursorStateChangedListener.onDragStarts(JTextView.this);
					}
					break;
				}
		...
		public void show(int x, int y) {
			final int[] coords = mTempCoords;
			JTextView.this.getLocationInWindow(coords);

			coords[0] += x - mHotspotX;
			coords[1] += y - mHotspotY;
			mContainer.showAtLocation(JTextView.this, Gravity.NO_GRAVITY, coords[0], coords[1]);
		}
		...
		private void onDetached() {
			...
			// I've made the action window a task of the View using JTextView instead of
